...
```bash
python3 -m venv .venv
.venv/bin/pip install -e '.[greenplum]'
```

Проверенный стек: Python 3.11, SQLGlot 25.34.x, psycopg2 2.9.x.
...
Готовый выполненный notebook: [`notebooks/sql_catalog_from_dataframe.ipynb`](notebooks/sql_catalog_from_dataframe.ipynb). Он принимает DataFrame с обязательными колонками `query_text`, `query_text_template` и необязательными `query_id`, `source_row_count`:
